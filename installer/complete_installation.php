						...
						if($sql_dump != ''){
							if(false == ($db_error = apphp_db_install($sql_dump_file))){
								...
								if(EI_USE_ADMIN_ACCOUNT){
									...
									$config_file = str_replace('<GLOBEADMIN_USERNAME>', $admin_username, $config_file);
									$config_file = str_replace('<GLOBEADMIN_PASSWORD>', $admin_password, $config_file);
								...
								if(is_file(EI_CONFIG_FILE_PATH)){
				...
				<?php if($install_type == 'update'){ ?>
					...
					<tr>
						<td>
							...
							<br /><br />
							<?php if(EI_APPLICATION_START_FILE != ''){ ?><a href="<?php echo '../'.EI_APPLICATION_START_FILE;?>" target="_blank"><?php echo lang_key('proceed_to_login_page'); ?></a><?php } ?>
						</td>
					</tr>									
